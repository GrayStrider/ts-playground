 ...
 */
interval(1000).pipe(auditTime(1010), take(10))
// .subscribe(value => console.log(value))
 ...
 * promisify axios
 */
const axiosAsync = new Promise<AxiosResponse>((resolve, reject) => {
	resolve(axios.get(hondas))
})

from(axiosAsync).pipe(map(result => result.status))
